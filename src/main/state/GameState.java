...
import main.component.visual_handler.Background;
import main.map.Map;
import main.map.map_objects.Arrow;
import main.map.map_objects.entity.Archer;
import main.map.map_objects.entity.Civilian;
import main.map.map_objects.entity.Player;
...
public class GameState implements State {
    ...
    public void update(float dt) {
        if (!StateManager.isPaused()) {
                        ...
                        if (!map.getBuildings().get(i).hasTakenDamage()) {
                            ...
                            if (map.getBuildings().get(i).isSinking()) {
                                for (int j = 0; j < new Random().nextInt(3) + 2; j++) { //people run out of the buildings when it starts sinking
                                    civilians.add(new Civilian(map.getBuildings().get(i).getX() + new Random().nextInt(75) + 100, Window.WINDOW_HEIGHT - 70));
                                }
                            }
                        }
            ...
            if ((int) player.getX() / (bg.getX() + 1) > 0){
                bg.updateX();
